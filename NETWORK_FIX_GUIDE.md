...
## Root Cause
The mobile app was hardcoded to connect to IP address `192.168.1.11:3000`, but the actual backend server was running on `10.198.136.226:3000`.
...
1. **SealTechnician/src/services/AuthService.ts**
   - Changed: `http://192.168.1.11:3000/api` → `http://10.198.136.226:3000/api`

2. **SealTechnician/src/services/TechnicianService.ts**
   - Changed: `http://192.168.1.11:3000/api` → `http://10.198.136.226:3000/api`

3. **SealTechnician/src/views/ScanScreen.tsx**
   - Changed: `http://192.168.1.11:3000/api/scan-seal` → `http://10.198.136.226:3000/api/scan-seal`

4. **SealClient/src/services/api.ts**
   - Updated to use the correct server IP: `http://10.198.136.226:3000`
...
Invoke-WebRequest -Uri "http://10.198.136.226:3000/api/technician/login" -Method POST -Headers @{"Content-Type"="application/json"} -Body $body
```

**Linux/macOS (curl):**
```bash
curl -X POST http://10.198.136.226:3000/api/technician/login \
...
### Login
```
POST http://10.198.136.226:3000/api/technician/login
...
```
GET http://10.198.136.226:3000/api/technician/my-seals
