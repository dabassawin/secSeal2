package main

import (
	"fmt"
	"log"
	"os"
	"time"

	"gorm.io/driver/postgres"
	"gorm.io/gorm"
	"gorm.io/gorm/logger"
	"github.com/joho/godotenv"
)

func main() {
	godotenv.Load(".env")
	dsn := fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=disable TimeZone=Asia/Bangkok",
		os.Getenv("DB_HOST"), os.Getenv("DB_PORT"), os.Getenv("DB_USER"),
		os.Getenv("DB_PASSWORD"), os.Getenv("DB_NAME"))

	db, err := gorm.Open(postgres.Open(dsn), &gorm.Config{
		Logger: logger.Default.LogMode(logger.Silent),
	})
	if err != nil {
		log.Fatal(err)
	}

	queryStr := `SELECT id, user_id, action, timestamp FROM logs ORDER BY id DESC LIMIT 10`
	rows, err := db.Raw(queryStr).Rows()
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()

	fmt.Println("Recent logs:")
	for rows.Next() {
		var id, userID int
		var action string
		var timestamp time.Time
		rows.Scan(&id, &userID, &action, &timestamp)
		fmt.Printf("- ID: %v, UserID: %v, Action: %v, Time: %v\n", id, userID, action, timestamp.Format("15:04:05"))
	}
}
